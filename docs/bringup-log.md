...
The first speech turn generated 57,600 samples from 32,768 captured samples in 1,074 ms, but the device correctly used local fallback. Diagnosis showed that a single upstream audio delta had been synchronously expanded into roughly 60 device frames, exceeding the intentionally small 33-event device queue. The Agent now queues at most the already-bounded six seconds of output and pumps 960-sample frames without blocking the OpenAI callback. Physical trials reduced pacing from 40 ms to 20 ms and then 10 ms. Although 10 ms initially produced several exact turns and felt faster, longer testing recorded three device protocol errors on an otherwise successful 82,800-sample generation and correctly fell back locally. The candidate therefore returns to the reliable 20 ms cadence; the display remained visible throughout.
...
A later buffered-streaming candidate negotiated its mode explicitly and began audible synthesized speech quickly from a 500 ms jitter threshold while retaining exact token/sequence validation. Its first physical reply was continuous for roughly six seconds, then stopped and showed a persistent error expression. Aggregate diagnostics proved that transport remained healthy; OpenAI had generated more than the then-fixed 30-second output allowance in about six seconds, triggering `output_audio_out_of_bounds`. The old Worker-upgrade close message observed in logs belonged to the replaced isolate and was not the turn failure.

The follow-up removes the complete-turn storage assumption: device PCM retention is now a circular 30-second window with bounded frame-sized copies and no bulk `memmove` in the audio task. Cloud delivery fills the jitter buffer at 20 ms/frame for 25 frames, then matches playback at 40 ms/frame; consumed Agent queue entries are reclaimed. Output has a five-minute provider abuse guard rather than a 30-second conversational cutoff, and the error expression returns to idle after 1.5 seconds. This rolling candidate remains unflashed pending the same cold-boot and physical gates.
